...
| TikTok installed | `adb shell pm list packages com.zhiliaoapp.musically` (override with `TIKTOK_ANDROID_PACKAGE`) |
...
- **`a11y-bridge`** — the sim-use bridge APK: an `AccessibilityService` plus a local HTTP server, so
  ...
  Press **Set up the driver** to read the token and run `adb forward tcp:18300 tcp:18300`. With the
  Wi-Fi build of the APK, put `http://<phone-ip>:18300` in the **Bridge URL** field instead and the
