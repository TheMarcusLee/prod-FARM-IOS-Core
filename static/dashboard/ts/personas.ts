...
function filter(search: HTMLInputElement): void {
    ...
    for (const group of form.querySelectorAll<HTMLElement>('[data-preset-group]')) {
        let visible = 0;
        for (const option of group.querySelectorAll<HTMLElement>('[data-preset-terms]')) {
    ...
    const empty = form.querySelector<HTMLElement>('[data-preset-empty]');
    if (empty) empty.hidden = shown > 0;
}

document.addEventListener('input', (event) => {
    const target = event.target;
    if (target instanceof HTMLInputElement && target.matches(SEARCH)) filter(target);
});
