...
function initCalibrate() {
    document.addEventListener('click', (event) => {
        const button = event.target?.closest('[data-calibrate]');
        if (!button)
            return;
        const { plugin, flow, udid } = button.dataset;
        if (!plugin || !flow || !udid)
            return;
        const result = pick('#calibrate-result');
        ...
        void send('/api/agent/calibrate', { plugin, flow, udid })
            ...
            if (result)
                result.textContent = `Calibration queued for ${plugin} ${flow} on ${udid}. Follow it in this phone's activity.`;
        })
            .catch((error) => {
