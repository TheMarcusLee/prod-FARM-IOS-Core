...
export async function registerMobileRoutes(app: FastifyInstance, options: MobileRouteOptions): Promise<void> {
    ...
    app.get('/api/mobile/bootstrap', async () => {
        ...
        return {
            ...
            // TODO: return acknowledgedCountFor(request.apiToken) from src/fleet/acks.ts
            // once the push/ack work lands; there is no ack store in this tree yet.
            unacknowledgedCount: 0,
            capabilities: {
                push: false, // flips to true when POST /api/push/register merges
                eventAck: false,
                thumbnails: true,
                contentQueue: true,
                tokens: true,
