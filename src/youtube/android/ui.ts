...
/**
 * The vocabulary the Android YouTube routines use to talk about on-screen controls.
 *
 * Same shape as the TikTok table (`src/tiktok/android/ui.ts`) and for the same reason: YouTube
 * relabels and re-ids things between builds, regions and A/B buckets, so every control is a *list*
 * of selectors tried in order — a `resource-id` fragment where one exists, visible text or
 * content-desc otherwise. The routine files keep those lists in one exported table at the top so a
 * wrong guess can be corrected against a real phone without touching the flow.
 */
...
export interface TapOptions {
    /** OCR fallback for the screens YouTube draws without accessibility nodes (the Shorts player). */
    recognize?: Recognize;
    /**
...
/**
 * Where to tap for the first matching selector. Tree first; OCR second, for the surfaces YouTube
...
/** Taps the first selector that resolves; a miss names the control, the alternates and the screen. */
...
/** Like `tapFirst`, but a missing control is not fatal — used for the optional screens. */
 ...
 * (tests drive the routines with a fake driver and never need it). The recognizer itself is a
 * plain wrapper around node-native-ocr and has nothing TikTok-specific in it; it simply lives
 * under `src/tiktok/` because that is where the first routine to need it was written.
 */
