...
/**
 * The vocabulary the Android Instagram routines use to talk about on-screen controls.
 *
 * Same shape as the TikTok table next door, and for the same reason: every control is a *list* of
 * selectors tried in order, because Instagram relabels and re-ids things between builds, regions
 * and A/B buckets. A selector is either a `resource-id` fragment (stable where it exists, absent
 * on Instagram's Compose-drawn surfaces) or visible text / content-desc. Selector lists live in
 * one exported table at the top of each routine so they can be corrected against a real phone
 * without touching the flow.
 *
 * It is deliberately a sibling of `src/tiktok/android/ui.ts` rather than a shared module: the
 * error messages name the app an operator is looking at, and one app's selector conventions
 * should never be able to break the other's routines.
 */
...
export interface TapOptions {
    /** OCR fallback for the screens Instagram draws without accessibility nodes. */
    recognize?: Recognize;
    /**
...
/**
 * Where to tap for the first matching selector. Tree first; OCR second, for the screens Instagram
 * draws without accessibility nodes (parts of the reel editor and the sticker tray do this).
 */
...
/** Taps the first selector that resolves, and logs where it landed. */
export async function tapFirst(driver: DeviceDriver, label: string, selectors: SelectorList, options: TapOptions = {}): Promise<void> {
    ...
    if (!point) {
        ...
        throw await missingControlError(driver, 'Instagram', label, selectors);
    }
...
/**
 * Like `tapFirst`, but a missing control is not fatal. This is what makes the flows tolerant:
 * Instagram shows "New reel" sheets, "Add audio" prompts, cookie-style upsells and first-run
 * tooltips on some builds and not others, and a routine that insisted on them would fail on every
 * phone that had already dismissed one.
 */
 ...
 * (tests drive the routines with a fake driver and never need it). Word recognition knows nothing
 * about which app produced the screenshot, so this shares the farm's one recognizer.
 */
