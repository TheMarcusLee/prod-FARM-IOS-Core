/**
 * The Backline glyphs, copied byte-for-byte from `src/ui/icons.ts` — the same
 * 16-unit grid, the same `d` attributes, rendered at 24 on the phone. They are
 * strings rather than JSX so a diff against the desktop set is exact; `Icon`
 * turns them into `react-native-svg` elements. Add a glyph *there* first.
 */
